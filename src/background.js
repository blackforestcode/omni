...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
	// Sicherheits-Check: Nur Nachrichten von eigenen Tabs akzeptieren
	if (!sender.tab || !sender.tab.id) {
		console.warn("Nachricht von unbekannter Quelle blockiert.");
		return;
	}
	// Bei kritischen Aktionen Nutzerbestätigung verlangen
	...
	if (criticalActions.includes(message.request)) {
		if (
			!confirm(
				"Achtung: Diese Aktion löscht persönliche Browserdaten unwiderruflich. Fortfahren?"
			)
		) {
			return;
		}
	...
	switch (message.request) {
		...
		case "unpin":
			...
			break;
		case "remove-all":
			requestOptionalPermission("browsingData", (granted) => {
				if (granted) clearAllData();
			});
			break;
		case "remove-history":
			requestOptionalPermission("browsingData", (granted) => {
				if (granted) clearBrowsingData();
			});
			break;
		case "remove-cookies":
			requestOptionalPermission("browsingData", (granted) => {
		...
		case "remove-cache":
			requestOptionalPermission("browsingData", (granted) => {
				if (granted) clearCache();
			});
			break;
		case "remove-local-storage":
			requestOptionalPermission("browsingData", (granted) => {
				if (granted) clearLocalStorage();
			});
			break;
		case "remove-passwords":
			requestOptionalPermission("browsingData", (granted) => {
				if (granted) clearPasswords();
			});
			break;
		...
		case "close-tab":
			...
			break;
		case "search-history":
			requestOptionalPermission("history", (granted) => {
				if (!granted) {
		...
		case "search-bookmarks":
			requestOptionalPermission("bookmarks", (granted) => {
				if (!granted) {
...
function requestOptionalPermission(permission, callback) {
	if (!chrome.permissions) return callback(true); // fallback für Browser ohne API
...
// Beispiel für History-Suche:
// requestOptionalPermission('history', (granted) => { if (granted) chrome.history.search(...); });
